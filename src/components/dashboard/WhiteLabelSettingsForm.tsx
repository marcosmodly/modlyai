...
export default function WhiteLabelSettingsForm({
  ...
  const apiKey = store.apiKey || fallbackApiKey || ''
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <section className="rounded-[32px] border border-stone-200 bg-white p-6 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <h2 className="text-2xl font-bold tracking-tight text-stone-950">Developer</h2>
            <p className="mt-1 text-sm text-stone-500">Use this key and install snippet to load the widget on your storefront.</p>
          </div>
          <Link
            href="/dashboard/integrations"
            className="inline-flex items-center gap-2 rounded-xl border border-stone-200 bg-white px-3 py-2 text-sm font-medium text-stone-700 transition hover:border-stone-300 hover:bg-stone-50"
          >
        ...
        <div className="mt-6 flex flex-col gap-2 sm:flex-row">
          <input
            ...
            value={apiKey}
            readOnly
            className="flex-1 rounded-2xl border border-stone-300 bg-stone-50 px-4 py-3 font-mono text-sm"
          />
          {apiKey ? <CopyButton value={apiKey} className="justify-center sm:min-w-28" /> : null}
        </div>
      </section>
