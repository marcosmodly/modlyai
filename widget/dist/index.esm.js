...
function FurnitureCustomizerPanel({ products, draft, setDraft, isApplying, validationErrors, price, onApply, onUndo, onRedo, canUndo, canRedo, onSaveConfig, onShareLink, onExportPdf, onViewFullRoomAnalysis, onSuggestionsError, onRequestQuote, showRequestQuote, primaryColor, }) {
    const selectedProduct = useMemo(() => products.find((p) => p.id === draft.productId) ?? products[0], [draft.productId, products]);
    const customization = useMemo(() => getProductCustomization(selectedProduct), [selectedProduct]);
    useEffect(() => {
    ...
    const [roomPlannerPhoto, setRoomPlannerPhoto] = useState(null);
    const [roomPlannerRecommendations, setRoomPlannerRecommendations] = useState(null);
    ...
    const compactSuggestions = useMemo(() => (aiSuggestions?.recommendations ?? []).slice(0, CUSTOMIZER_SUGGESTION_LIMIT), [aiSuggestions]);
...
const FurnitureCustomizerWidget = forwardRef(function FurnitureCustomizerWidget({ config = {}, onNavigateToRoomPlanner, selectedProduct: sharedSelectedProduct, onSelectedProductChange, onQuoteSubmitted }, ref) {
    ...
    const defaultProduct = useMemo(() => sharedSelectedProduct ?? availableProducts[0] ?? products[0], [availableProducts, sharedSelectedProduct]);
    const [, setCustomizedItem] = useState(null);
    ...
    const [baseProduct, setBaseProduct] = useState(defaultProduct);
    const [draft, setDraft] = useState(() => createDraftForProduct(defaultProduct));
    const [history, setHistory] = useState(() => [createDraftForProduct(defaultProduct)]);
    ...
    const setDraftWithHistory = useCallback((next) => {
        setDraft(next);
        setSavedItem(null);
    ...
    const applySelectedProduct = useCallback((product, clearSessionStorage = false) => {
    ...
    const selectedProduct = useMemo(() => availableProducts.find((product) => product.id === draft.productId) ?? baseProduct, [availableProducts, baseProduct, draft.productId]);
    useEffect(() => {
    ...
    const configStorageKey = useMemo(() => `${mergedConfig.storageKey}:customizer-configs`, [mergedConfig.storageKey]);
    ...
    const buildCustomizationConfig = useCallback(() => {
        const customization = getCustomizationForProduct(selectedProduct);
    ...
    const buildCustomizedFurniturePayload = useCallback((customizedData) => {
        const customization = getCustomizationForProduct(selectedProduct);
        const selectedColor = getSelectedColorOption(selectedProduct, draft);
        const selectedMaterial = customization.materials.find((material) => draft.selectedMaterial
            ? material.name.toLowerCase() === draft.selectedMaterial.toLowerCase()
    ...
    const saveCustomizedFurnitureForCurrentDraft = useCallback((customizedData) => {
...
function FurnitureAIWidgetButton({ config = {}, defaultTab = 'room-planner', buttonText, buttonPosition = 'bottom-right', buttonStyle, className = '' }) {
    ...
    const [entryTab, setEntryTab] = useState('conversation');
