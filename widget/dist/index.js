...
function RoomUploadForm({ onUpload, isLoading, onPhotosChange, formRef, initialDimensions, initialPreferences, onDimensionsChange, onPreferencesChange, }) {
    const formElementRef = require$$0.useRef(null);
    ...
    const [photos, setPhotos] = require$$0.useState([]);
    const [unitSystem, setUnitSystem] = require$$0.useState('meters');
    const [dimensions, setDimensions] = require$$0.useState(getInitialDimensions());
    ...
    const [lengthFeet, setLengthFeet] = require$$0.useState(initialFeetInches.lengthFI.feet);
    const [lengthInches, setLengthInches] = require$$0.useState(initialFeetInches.lengthFI.inches);
    const [widthFeet, setWidthFeet] = require$$0.useState(initialFeetInches.widthFI.feet);
    const [widthInches, setWidthInches] = require$$0.useState(initialFeetInches.widthFI.inches);
    const [heightFeet, setHeightFeet] = require$$0.useState(initialFeetInches.heightFI.feet);
    const [heightInches, setHeightInches] = require$$0.useState(initialFeetInches.heightFI.inches);
    const [preferences, setPreferences] = require$$0.useState(initialPreferences || {});
...
function RecommendationsList({ recommendations, onCustomize, onFinalize }) {
    ...
    const [showDetailsModal, setShowDetailsModal] = require$$0.useState(false);
...
function RoomPhotoPreview({ photoUrl, showFurniture = false }) {
    const [isFullscreen, setIsFullscreen] = require$$0.useState(false);
...
function ColorComboBox({ label, value, onChange, options, placeholder = '', disabled = false, }) {
    ...
    const [filteredOptions, setFilteredOptions] = require$$0.useState(options);
...
function FurnitureCustomizerPanel({ baseItem, onCustomize, isLoading, }) {
    ...
    const [customPrompt, setCustomPrompt] = require$$0.useState('');
    const handleSubmit = (e) => {
        e.preventDefault();
        onCustomize(config);
    };
...
function FurnitureCustomizerWidget({ config = {}, onNavigateToRoomPlanner }) {
    ...
    const [selectedMaterial, setSelectedMaterial] = require$$0.useState('fabric');
    const [baseItem, setBaseItem] = require$$0.useState({
        id: 'custom-1',
...
function FurnitureAIWidget({ config = {}, defaultTab }) {
    ...
    const apiClient = require$$0.useMemo(() => new ApiClient(mergedConfig), [mergedConfig]);
    const storage = require$$0.useMemo(() => new Storage(mergedConfig.storageKey), [mergedConfig.storageKey]);
    const aiServiceRef = require$$0.useRef(null);
    ...
    const [customizeItem, setCustomizeItem] = require$$0.useState(null);
    const [saveNotification, setSaveNotification] = require$$0.useState(null);
