...
const LucideContext = require$$0.createContext({});
const useLucideContext = () => require$$0.useContext(LucideContext);
...
const Lamp = createLucideIcon("lamp", __iconNode$i);
...
class ApiClient {
    ...
    normalizeCatalogProductForChat(product, index) {
        ...
        return {
            ...
            colors: normalizeStringList$1(product?.colors),
            materials: normalizeStringList$1(product?.materials),
...
const WidgetContext = require$$0.createContext(null);
...
function useWidgetContext() {
    const context = require$$0.useContext(WidgetContext);
    if (!context) {
...
function buildProductUrl(domain, handle, platform, productUrlTemplate) {
...
function buildSearchUrl(domain, term, platform) {
...
function normalizeDomain(storeDomain) {
    if (!storeDomain)
        return undefined;
...
function getRealProductUrl(product, storeDomain, platform, productUrlTemplate) {
    ...
    if (handle) {
        if (productUrlTemplate) {
            ...
            if (built && isRealProductUrl(built))
                return built;
        }
        if (domain) {
    ...
    if (name) {
        ...
        if (search && isRealProductUrl(search))
            return search;
    }
    return undefined;
}
...
function CustomizedFurnitureList({ items, onItemRemoved, onNavigateToCustomizer, onRequestQuote, highlightItemId, }) {
    ...
    const [removingId, setRemovingId] = require$$0.useState(null);
    const [pendingRemoveId, setPendingRemoveId] = require$$0.useState(null);
    const [imageErrors, setImageErrors] = require$$0.useState(() => new Set());
    ...
    return (jsxRuntimeExports.jsxs("div", { className: "space-y-5", children: [jsxRuntimeExports.jsxs("div", { children: [jsxRuntimeExports.jsx("h2", { className: "text-2xl font-bold text-gray-950", children: "My Customized Furniture" }), jsxRuntimeExports.jsxs("p", { className: "mt-1 text-sm text-gray-500", children: [items.length, " saved custom design", items.length === 1 ? '' : 's'] })] }), jsxRuntimeExports.jsx("div", { className: ['grid grid-cols-1 gap-5', items.length === 1 ? 'mx-auto max-w-md' : 'sm:grid-cols-2 xl:grid-cols-3'].join(' '), children: items.map((item) => {
                    ...
                    return (jsxRuntimeExports.jsxs("article", { id: `customized-item-${item.id}`, className: [
...
function useWebsiteColors() {
    const [colors, setColors] = require$$0.useState({
...
function FinalizeQuoteModal({ isOpen, onClose, onProceed, item, recommendation, roomDimensions, roomAnalysis, }) {
    ...
    const [inlineError, setInlineError] = require$$0.useState(null);
    if (!isOpen)
        return null;
...
function QuoteRequestForm({ isOpen, onClose, onSubmit, item, recommendation, storeId, widgetId, roomDimensions, roomAnalysis, }) {
    const websiteColors = useWebsiteColors();
    const [formData, setFormData] = require$$0.useState(emptyForm);
    const [isSubmitting, setIsSubmitting] = require$$0.useState(false);
    const [error, setError] = require$$0.useState(null);
    const [success, setSuccess] = require$$0.useState(null);
    require$$0.useEffect(() => {
        if (!isOpen) {
...
function getProductCatalogUrl$1(item, storeDomain, platform, productUrlTemplate) {
    return getRealProductUrl(item, storeDomain, platform, productUrlTemplate);
}
function getProductImageUrl$2(item) {
...
function RecommendationsList({ recommendations, onCustomize, onFinalize, enabledActions, primaryColor, analyticsContext, storeDomain, platform, productUrlTemplate }) {
    ...
    const [lightbox, setLightbox] = require$$0.useState(null);
    ...
    return (jsxRuntimeExports.jsxs("div", { className: "space-y-6", children: [jsxRuntimeExports.jsxs("div", { children: [jsxRuntimeExports.jsx("h2", { className: "text-3xl font-bold text-gray-950", children: "Matching catalog products" }), jsxRuntimeExports.jsx("p", { className: "mt-2 text-sm text-gray-600", children: "Suggestions are based on the uploaded room photo, your preferences, and available catalog data." })] }), jsxRuntimeExports.jsx("div", { className: "grid md:grid-cols-2 lg:grid-cols-3 gap-6", children: recommendations.map((rec, index) => {
                    const catalogUrl = getProductCatalogUrl$1(rec.item, storeDomain, platform, productUrlTemplate);
                    const imageUrl = getProductImageUrl$2(rec.item);
...
function FurnitureRoomPlannerWidget({ config = {}, onCustomizeItem, onNavigateToCustomizer, sampleRooms, onQuoteSubmitted, }) {
    const mergedConfig = require$$0.useMemo(() => mergeConfig(config), [config]);
    const apiClient = require$$0.useMemo(() => new ApiClient(mergedConfig), [mergedConfig]);
    const storage = require$$0.useMemo(() => new Storage(mergedConfig.storageKey), [mergedConfig.storageKey]);
    const enabledActions = require$$0.useMemo(() => getEnabledActions(mergedConfig), [mergedConfig]);
    const analyticsContext = require$$0.useMemo(() => ({
    ...
    const [recommendations, setRecommendations] = require$$0.useState(null);
    const [isLoading, setIsLoading] = require$$0.useState(false);
    const [error, setError] = require$$0.useState(null);
    const [shareMessage, setShareMessage] = require$$0.useState(null);
    const [resultsScrollRequest, setResultsScrollRequest] = require$$0.useState(0);
    const [uploadedPhotos, setUploadedPhotos] = require$$0.useState([]);
    const [customizedFurniture, setCustomizedFurniture] = require$$0.useState([]);
    const [savedDimensions, setSavedDimensions] = require$$0.useState(undefined);
    const [savedPreferences, setSavedPreferences] = require$$0.useState(undefined);
    const [roomType, setRoomType] = require$$0.useState('living');
    const [unitSystem, setUnitSystem] = require$$0.useState('meters');
    const [lengthValue, setLengthValue] = require$$0.useState('');
    const [widthValue, setWidthValue] = require$$0.useState('');
    const [heightValue, setHeightValue] = require$$0.useState('2.4');
    const [colorInput, setColorInput] = require$$0.useState('');
    const [budgetMin, setBudgetMin] = require$$0.useState('');
    const [budgetMax, setBudgetMax] = require$$0.useState('');
    const [selectedStyles, setSelectedStyles] = require$$0.useState(['modern']);
    const [showFinalizeModal, setShowFinalizeModal] = require$$0.useState(false);
    const [showQuoteForm, setShowQuoteForm] = require$$0.useState(false);
    const [quoteSuccess, setQuoteSuccess] = require$$0.useState(false);
    const [selectedRecommendation, setSelectedRecommendation] = require$$0.useState(null);
    const [selectedCustomizedItem, setSelectedCustomizedItem] = require$$0.useState(null);
    const [highlightItemId, setHighlightItemId] = require$$0.useState(null);
    const [lightboxImage, setLightboxImage] = require$$0.useState(null);
    const fileInputRef = require$$0.useRef(null);
    const resultsRef = require$$0.useRef(null);
    const lastScrolledRequestRef = require$$0.useRef(0);
    require$$0.useEffect(() => {
        trackWidgetEvent({
            ...analyticsContext,
...
const getProductImageUrl$1 = (product) => {
    if (!product)
        return undefined;
...
function FurnitureCustomizerPanel({ products, draft, setDraft, isApplying, validationErrors, price, onApply, onUndo, onRedo, canUndo, canRedo, onSaveConfig, onShareLink, onExportPdf, onViewFullRoomAnalysis, onSuggestionsError, onRequestQuote, showRequestQuote, primaryColor, }) {
    const selectedProduct = require$$0.useMemo(() => products.find((p) => p.id === draft.productId) ?? products[0], [draft.productId, products]);
    const customization = require$$0.useMemo(() => getProductCustomization(selectedProduct), [selectedProduct]);
    require$$0.useEffect(() => {
    ...
    const [additionalDetails, setAdditionalDetails] = require$$0.useState('');
    const [charCount, setCharCount] = require$$0.useState(0);
    const [roomPlannerPhoto, setRoomPlannerPhoto] = require$$0.useState(null);
    const [roomPlannerRecommendations, setRoomPlannerRecommendations] = require$$0.useState(null);
    const [aiSuggestions, setAiSuggestions] = require$$0.useState(null);
    const [analyzing, setAnalyzing] = require$$0.useState(false);
    const [aiSuggestionsOpen, setAiSuggestionsOpen] = require$$0.useState(false);
    const [productImageErrors, setProductImageErrors] = require$$0.useState(() => new Set());
    const [lightboxImage, setLightboxImage] = require$$0.useState(null);
    const roomPlannerSuggestions = require$$0.useMemo(() => {
    ...
    const compactSuggestions = require$$0.useMemo(() => (aiSuggestions?.recommendations ?? []).slice(0, CUSTOMIZER_SUGGESTION_LIMIT), [aiSuggestions]);
    ...
    return (jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [jsxRuntimeExports.jsx("section", { className: "py-12 bg-gray-50", children: jsxRuntimeExports.jsxs("div", { className: "max-w-7xl mx-auto px-4", children: [jsxRuntimeExports.jsxs("button", { type: "button", onClick: () => setAiSuggestionsOpen((prev) => !prev), "aria-pressed": aiSuggestionsOpen, className: [
...
- Must fit through a 32 inch doorway`, className: "w-full border border-gray-300 rounded-xl px-4 py-3 text-sm text-gray-700 resize-none focus:outline-none focus:ring-2 focus:ring-blue-500 placeholder-gray-400" }), jsxRuntimeExports.jsxs("div", { className: "flex items-center justify-between mt-2", children: [jsxRuntimeExports.jsxs("span", { className: "text-xs text-gray-400", children: [charCount, "/500 characters"] }), additionalDetails && (jsxRuntimeExports.jsx("button", { onClick: () => {
                                                                    ...
                                                                    }, className: "text-xs bg-gray-100 hover:bg-blue-50 hover:text-blue-600 text-gray-600 px-3 py-1 rounded-full transition-colors", children: ["+ ", chip] }, chip))) })] })] }), jsxRuntimeExports.jsxs("button", { type: "button", onClick: onApply, disabled: isApplying || (validationErrors?.length ?? 0) > 0, className: "w-full py-4 bg-purple-600 text-white rounded-full font-semibold hover:bg-purple-700 transition shadow-lg shadow-purple-600/30 flex items-center justify-center gap-2 disabled:opacity-60 disabled:cursor-not-allowed", children: [jsxRuntimeExports.jsx(Sparkles, { className: "w-5 h-5" }), isApplying ? 'Applying...' : 'Apply Customizations'] }), jsxRuntimeExports.jsxs("div", { className: "mt-3 grid grid-cols-2 gap-3", children: [showRequestQuote && (jsxRuntimeExports.jsx("button", { type: "button", onClick: onRequestQuote, disabled: isApplying, className: "min-h-12 rounded-lg px-4 py-3 text-sm font-semibold text-white shadow-sm transition hover:opacity-95 disabled:opacity-60", style: { backgroundColor: primaryColor ?? '#2563eb' }, children: "Add to Quote" })), jsxRuntimeExports.jsx("button", { type: "button", onClick: handleViewFullRoomAnalysis, className: [
...
function ActionToast({ toast, onDismiss }) {
    if (!toast)
        return null;
    return (jsxRuntimeExports.jsx("div", { className: "fixed top-4 left-1/2 z-[60] w-[calc(100%-2rem)] max-w-sm -translate-x-1/2 modly-panel-fade", role: "status", "aria-live": "polite", children: jsxRuntimeExports.jsxs("div", { className: `flex items-center gap-3 rounded-xl border px-4 py-3 shadow-lg ${TOAST_STYLES[toast.type]}`, children: [toast.type === 'loading' && jsxRuntimeExports.jsx(LoaderCircle, { className: "h-5 w-5 shrink-0 animate-spin" }), toast.type === 'success' && jsxRuntimeExports.jsx(CircleCheck, { className: "h-5 w-5 shrink-0" }), toast.type === 'error' && jsxRuntimeExports.jsx(CircleAlert, { className: "h-5 w-5 shrink-0" }), jsxRuntimeExports.jsx("p", { className: "flex-1 text-sm font-medium leading-snug", children: toast.message }), toast.type !== 'loading' && (jsxRuntimeExports.jsx("button", { type: "button", onClick: onDismiss, className: "shrink-0 rounded-full p-1 transition hover:bg-black/5", "aria-label": "Dismiss notification", children: jsxRuntimeExports.jsx(X, { className: "h-4 w-4" }) }))] }) }, toast.id));
}
...
const FurnitureCustomizerWidget = require$$0.forwardRef(function FurnitureCustomizerWidget({ config = {}, onNavigateToRoomPlanner, selectedProduct: sharedSelectedProduct, onSelectedProductChange, onQuoteSubmitted }, ref) {
    const mergedConfig = require$$0.useMemo(() => mergeConfig(config), [config]);
    const apiClient = require$$0.useMemo(() => new ApiClient(mergedConfig), [mergedConfig]);
    const storage = require$$0.useMemo(() => new Storage(mergedConfig.storageKey), [mergedConfig.storageKey]);
    const enabledActions = require$$0.useMemo(() => getEnabledActions(mergedConfig), [mergedConfig]);
    ...
    const primaryColor = getPrimaryColor(mergedConfig);
    const [catalogProducts, setCatalogProducts] = require$$0.useState([]);
    const availableProducts = require$$0.useMemo(() => {
    ...
    const defaultProduct = require$$0.useMemo(() => sharedSelectedProduct ?? availableProducts[0] ?? products[0], [availableProducts, sharedSelectedProduct]);
    const [, setCustomizedItem] = require$$0.useState(null);
    const [savedItem, setSavedItem] = require$$0.useState(null);
    const [isLoading, setIsLoading] = require$$0.useState(false);
    const [toast, setToast] = require$$0.useState(null);
    const toastTimeoutRef = require$$0.useRef(null);
    const toastIdRef = require$$0.useRef(0);
    const [lastConfig, setLastConfig] = require$$0.useState(null);
    const [baseProduct, setBaseProduct] = require$$0.useState(defaultProduct);
    const [draft, setDraft] = require$$0.useState(() => createDraftForProduct(defaultProduct));
    const [history, setHistory] = require$$0.useState(() => [createDraftForProduct(defaultProduct)]);
    const [historyIndex, setHistoryIndex] = require$$0.useState(0);
    const [showFinalizeModal, setShowFinalizeModal] = require$$0.useState(false);
    const [showQuoteForm, setShowQuoteForm] = require$$0.useState(false);
    const dismissToast = require$$0.useCallback(() => {
    ...
    const showToast = require$$0.useCallback((type, message) => {
        if (toastTimeoutRef.current) {
            clearTimeout(toastTimeoutRef.current);
            toastTimeoutRef.current = null;
        }
    ...
    const setDraftWithHistory = require$$0.useCallback((next) => {
        setDraft(next);
        setSavedItem(null);
    ...
    const applySelectedProduct = require$$0.useCallback((product, clearSessionStorage = false) => {
    ...
    const handleUndo = require$$0.useCallback(() => {
        setSavedItem(null);
        setCustomizedItem(null);
        setLastConfig(null);
        setHistoryIndex((idx) => {
    ...
    const handleRedo = require$$0.useCallback(() => {
        setSavedItem(null);
        setCustomizedItem(null);
        setLastConfig(null);
        setHistoryIndex((idx) => {
    ...
    const selectedProduct = require$$0.useMemo(() => availableProducts.find((product) => product.id === draft.productId) ?? baseProduct, [availableProducts, baseProduct, draft.productId]);
    require$$0.useEffect(() => {
    ...
    const validationErrors = require$$0.useMemo(() => {
        const errs = [];
    ...
    const configStorageKey = require$$0.useMemo(() => `${mergedConfig.storageKey}:customizer-configs`, [mergedConfig.storageKey]);
    const saveDraftConfig = require$$0.useCallback(() => {
        try {
            if (typeof window === 'undefined')
                return;
    ...
    const shareProductLink = require$$0.useMemo(() => {
    ...
    const buildCustomizationPdfBlob = require$$0.useCallback(() => {
        const customization = getCustomizationForProduct(selectedProduct);
        const selectedColor = getSelectedColorOption(selectedProduct, draft);
        const selectedMaterial = customization.materials.find((material) => draft.selectedMaterial
            ? material.name.toLowerCase() === draft.selectedMaterial.toLowerCase()
    ...
    const exportAsPdf = require$$0.useCallback(() => {
        if (typeof window === 'undefined')
            return;
        try {
    ...
    const handleShare = require$$0.useCallback(async () => {
        if (typeof window === 'undefined')
            return;
        let pdfFile;
        try {
    ...
    const buildCustomizationConfig = require$$0.useCallback(() => {
        const customization = getCustomizationForProduct(selectedProduct);
    ...
    const buildCustomizedFurniturePayload = require$$0.useCallback((customizedData) => {
        const customization = getCustomizationForProduct(selectedProduct);
        const selectedColor = getSelectedColorOption(selectedProduct, draft);
        const selectedMaterial = customization.materials.find((material) => draft.selectedMaterial
            ? material.name.toLowerCase() === draft.selectedMaterial.toLowerCase()
    ...
    const saveCustomizedFurnitureForCurrentDraft = require$$0.useCallback((customizedData) => {
    ...
    const saveDraftAndHighlight = require$$0.useCallback(() => {
    ...
    require$$0.useImperativeHandle(ref, () => ({ saveDraftAndHighlight }), [saveDraftAndHighlight]);
    const handleCustomize = require$$0.useCallback(async (customizationConfig) => {
        ...
        try {
            ...
            const errorMessage = customizeError instanceof Error ? customizeError.message : "Couldn't apply changes. Try again.";
    ...
    const handleApply = require$$0.useCallback(() => {
...
function MessageBubble({ message, onCustomizeItem, onAddToRoomPlanner, onViewInCatalog, enabledActions, primaryColor, messageTextColor, analyticsContext, storeDomain, platform, productUrlTemplate }) {
...
function ConversationInterface({ aiService, onCustomizeItem, onAddToRoomPlanner, onOpenRoomPlanner, onOpenCustomizer, onShowCatalog, onViewInCatalog, enabledActions, primaryColor, messageTextColor, analyticsContext, storeDomain, platform, productUrlTemplate, suggestedPrompts, }) {
    const [input, setInput] = require$$0.useState('');
    const [isLoading, setIsLoading] = require$$0.useState(false);
    const [messages, setMessages] = require$$0.useState([]);
    const messagesContainerRef = require$$0.useRef(null);
...
function SubmitFlowModal({ config, product, apiClient, onSuccess, onClose, }) {
    const [step, setStep] = require$$0.useState('validate');
    const [flowType, setFlowType] = require$$0.useState('quote');
    const [validation, setValidation] = require$$0.useState(null);
    const [specSheet, setSpecSheet] = require$$0.useState(null);
    const [error, setError] = require$$0.useState(null);
    // Form fields for quote request
    const [customerName, setCustomerName] = require$$0.useState('');
    const [customerEmail, setCustomerEmail] = require$$0.useState('');
    const [customerPhone, setCustomerPhone] = require$$0.useState('');
    const [customerNotes, setCustomerNotes] = require$$0.useState('');
    require$$0.useEffect(() => {
...
function FurnitureAIWidget({ config = {}, defaultTab, widgetTitle, hideNav, initialProduct, sampleRooms, suggestedPrompts, onQuoteSubmitted, }) {
    ...
    const apiClient = require$$0.useMemo(() => new ApiClient(mergedConfig), [serviceConfigKey]);
    const storage = require$$0.useMemo(() => new Storage(mergedConfig.storageKey), [mergedConfig.storageKey]);
    const aiService = require$$0.useMemo(() => new AIService(apiClient, mergedConfig), [apiClient, serviceConfigKey]);
    const enabledActions = require$$0.useMemo(() => getEnabledActions(mergedConfig), [mergedConfig]);
    const analyticsContext = require$$0.useMemo(() => ({
        apiBaseUrl: mergedConfig.apiBaseUrl,
        storeId: mergedConfig.storeId || mergedConfig.widgetId,
    ...
    const [selectedProduct, setSelectedProduct] = require$$0.useState(initialProduct ? productFromFurnitureItem(initialProduct) : null);
    ...
    const [isCatalogModalOpen, setIsCatalogModalOpen] = require$$0.useState(false);
    const [showSubmitModal, setShowSubmitModal] = require$$0.useState(false);
    const [submitConfig, setSubmitConfig] = require$$0.useState(null);
    require$$0.useEffect(() => {
    ...
    const customizerRef = require$$0.useRef(null);
...
exports.FurnitureAIWidgetButton = FurnitureAIWidgetButton;
exports.FurnitureCustomizerWidget = FurnitureCustomizerWidget;
exports.FurnitureRoomPlannerWidget = FurnitureRoomPlannerWidget;
exports.SpecSheetPreview = SpecSheetPreview;
