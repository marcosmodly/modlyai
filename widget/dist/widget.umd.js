(function (factory) {
	...
	async function fetchRemoteConfig(configUrl, widgetId, storeId, shop) {
	...
	const metersToInches$1 = (value) => Number((value / 0.0254).toFixed(1));
	const inchesToMeters$1 = (value) => Number((value * 0.0254).toFixed(3));
	...
	const dimensionsFromInches = (lengthIn, widthIn, heightIn) => ({
	    length: inchesToMeters$1(lengthIn),
	    width: inchesToMeters$1(widthIn),
	    height: inchesToMeters$1(heightIn),
	    unit: 'm',
	});
	const products = [
	    {
	...
	function productFromFurnitureItem(item) {
	    ...
	    const widthIn = Math.round(metersToInches$1(item.dimensions.width || 0.9));
	    const depthIn = Math.round(metersToInches$1(item.dimensions.depth || item.dimensions.length || 0.9));
	    return {
	        id: item.id,
	        name: item.name,
	        category: item.category,
	        basePrice: item.priceRange?.min ?? 0,
	        length: rawItem.length ?? metersToInches$1(item.dimensions.length || item.dimensions.depth || 0),
	        width: rawItem.width ?? metersToInches$1(item.dimensions.width || 0),
	        height: rawItem.height ?? metersToInches$1(item.dimensions.height || 0),
	        source: item.source,
	...
	function CustomizedFurnitureList({ items, onItemRemoved, onNavigateToCustomizer, onRequestQuote, highlightItemId, }) {
	    ...
	    const [pendingRemoveId, setPendingRemoveId] = reactExports.useState(null);
	...
	const metersToInches = (meters) => meters * INCHES_PER_METER;
	const inchesToMeters = (inches) => inches / INCHES_PER_METER;
	...
	function FurnitureRoomPlannerWidget({ config = {}, onCustomizeItem, onNavigateToCustomizer, sampleRooms, onQuoteSubmitted, }) {
	    ...
	    const [highlightItemId, setHighlightItemId] = reactExports.useState(null);
	...
	function FurnitureCustomizerPanel({ products, draft, setDraft, isApplying, validationErrors, price, onApply, onUndo, onRedo, canUndo, canRedo, onSaveConfig, onShareLink, onExportPdf, onViewFullRoomAnalysis, onSuggestionsError, onRequestQuote, showRequestQuote, primaryColor, }) {
	    ...
	    const [aiSuggestionsOpen, setAiSuggestionsOpen] = reactExports.useState(false);
	    const [productImageErrors, setProductImageErrors] = reactExports.useState(() => new Set());
	    const [lightboxImage, setLightboxImage] = reactExports.useState(null);
...
- Must fit through a 32 inch doorway`, className: "w-full border border-gray-300 rounded-xl px-4 py-3 text-sm text-gray-700 resize-none focus:outline-none focus:ring-2 focus:ring-blue-500 placeholder-gray-400" }), jsxRuntimeExports.jsxs("div", { className: "flex items-center justify-between mt-2", children: [jsxRuntimeExports.jsxs("span", { className: "text-xs text-gray-400", children: [charCount, "/500 characters"] }), additionalDetails && (jsxRuntimeExports.jsx("button", { onClick: () => {
	...
	const FurnitureCustomizerWidget = reactExports.forwardRef(function FurnitureCustomizerWidget({ config = {}, onNavigateToRoomPlanner, selectedProduct: sharedSelectedProduct, onSelectedProductChange, onQuoteSubmitted }, ref) {
	    const mergedConfig = reactExports.useMemo(() => mergeConfig(config), [config]);
	    const apiClient = reactExports.useMemo(() => new ApiClient(mergedConfig), [mergedConfig]);
	    const storage = reactExports.useMemo(() => new Storage(mergedConfig.storageKey), [mergedConfig.storageKey]);
	    const enabledActions = reactExports.useMemo(() => getEnabledActions(mergedConfig), [mergedConfig]);
	    ...
	    const showToast = reactExports.useCallback((type, message) => {
	        if (toastTimeoutRef.current) {
	            clearTimeout(toastTimeoutRef.current);
	            toastTimeoutRef.current = null;
	        }
	    ...
	    const saveDraftAndHighlight = reactExports.useCallback(() => {
	    ...
	    reactExports.useImperativeHandle(ref, () => ({ saveDraftAndHighlight }), [saveDraftAndHighlight]);
...
<body>
  ...
  `.trim();
	...
	function FurnitureAIWidget({ config = {}, defaultTab, widgetTitle, hideNav, initialProduct, sampleRooms, suggestedPrompts, onQuoteSubmitted, }) {
	    ...
	    const apiClient = reactExports.useMemo(() => new ApiClient(mergedConfig), [serviceConfigKey]);
	    const storage = reactExports.useMemo(() => new Storage(mergedConfig.storageKey), [mergedConfig.storageKey]);
	    const aiService = reactExports.useMemo(() => new AIService(apiClient, mergedConfig), [apiClient, serviceConfigKey]);
	    const enabledActions = reactExports.useMemo(() => getEnabledActions(mergedConfig), [mergedConfig]);
	    const analyticsContext = reactExports.useMemo(() => ({
	        apiBaseUrl: mergedConfig.apiBaseUrl,
	        storeId: mergedConfig.storeId || mergedConfig.widgetId,
	    ...
	    const [viewMode, setViewMode] = reactExports.useState(defaultTab || 'conversation');
	    const [selectedProduct, setSelectedProduct] = reactExports.useState(initialProduct ? productFromFurnitureItem(initialProduct) : null);
	    ...
	    const customizerRef = reactExports.useRef(null);
	...
	function FurnitureAIWidgetButton({ config = {}, defaultTab = 'room-planner', buttonText, buttonPosition = 'bottom-right', buttonStyle, className = '' }) {
	    ...
	    const [entryTab, setEntryTab] = reactExports.useState('conversation');
